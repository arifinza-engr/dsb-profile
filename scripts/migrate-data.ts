...
async function migrateData() {
  try {
    console.log("Starting data migration with transaction...");
    ...
    await db.transaction(async (tx) => {
      ...
      for (const member of data.company.organizationalStructure) {
        await tx.insert(organizationalStructure).values({
          name: member.name,
      ...
      for (const job of data.jobVacancies.china) {
        await tx.insert(jobVacancies).values({
          title: job.title,
      ...
      for (const service of data.servicesPage.additionalServices) {
        await tx.insert(serviceAdditionalServices).values({
          title: service.title,
          description: service.description,
          features: JSON.stringify(service.features),
        });
      ...
      for (const req of data.servicesPage.qualifications.requirements) {
        await tx.insert(serviceQualifications).values({
          type: "requirements",
          title: "Requirements",
          description: req,
        });
      ...
      await tx.insert(homepageHero).values({
      ...
      await tx.insert(homepageCompanyProfile).values({
      ...
      for (const item of data.homepage.whyChooseUs) {
        await tx.insert(homepageWhyChooseUs).values({
          icon: item.icon,
          title: item.title,
          description: item.description,
        });
      ...
      await tx.insert(homepageCta).values({
